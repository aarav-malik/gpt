...
export default function GalleryScreen({ navigation }) {
  const { broadcastPhoto, requestHD, service } = usePeerService();
  const { photos, loadPhotos, addRemotePhoto } = useStorage(broadcastPhoto);

  useEffect(() => {
    loadPhotos();
    const onPhoto = (photo) => addRemotePhoto(photo);
    service.on('photo', onPhoto);
    return () => service.off('photo', onPhoto);
  }, []);
