...
1. Install [Node.js](https://nodejs.org/) (v18 recommended). The Expo CLI is bundled locally so no global install is required.
...
3. Start the project for iOS, Android or Web:
   ```bash
   npm run dev       # opens Expo in interactive mode
   npm run ios
   npm run android
   npm run web
   ```
